...
> [!question]- What does it mean for a site to be static?
> A static site is a website that is made up of files that are downloaded and rendered by a browser, like HTML, CSS, and JavaScript. These files are "static" because their contents don't change often. The alternative would be a _dynamic_ site, which uses a server to assemble the same files on-the-fly. Static sites are faster and cheaper to host, and are perfectly servicable for sites where everyone sees the same thing.
...
>
> This blog post is written in Markdown, and you can see how it looks in the [source code](https://github.com/the-snesler/samnesler.com/tree/main/posts/portfolio.mdx)! You'll notice the extension is `.mdx`, which is a special version of Markdown that allows you to use React components inside your Markdown files. This post doesn't use that feature, but Astro supports MDX too.

> [!question]- What's a CMS?
> A CMS (Content Management System) is a software application that allows you to create, manage, and modify content on a website without needing to write code. For example, WordPress and Contentful are popular CMSes. Usually if you want to customize a site beyond included themes, you need to dig into the code anyway. I've built Astro sites that are driven by a CMS, but that's more complicated than we have time for today.
...
>
> Cloudflare is another company that does free static hosting, and in their case they serve something like 20% of all websites through their CDN. For them, the marginal cost of storing a couple megabytes of data on the CDN that other people are already paying for is negligible, so they can afford to give it away for free. There are also many more benefits, so much so that they wrote a blog post about it: https://blog.cloudflare.com/cloudflares-commitment-to-free
>
> This site is currently hosted on Cloudflare Workers, which is a free _dynamic_ hosting service that can run server-side code. That's how things like the guestbook work, since I need a very small database to store the drawings.
...
- If you use Discord, you might consider setting up my [Discord GitHub Preview project](https://github.com/the-snesler/discord-github-preview) :). It'll show your Discord activity in an SVG that you can place in an `<img>` tag. This is a great way to show off your activity and make your site more dynamic.
