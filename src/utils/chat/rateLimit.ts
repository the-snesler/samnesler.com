...
export async function checkRateLimit(
  ...
  // Cloudflare's edge always sets CF-Connecting-IP; its absence means the request never went
  // through the edge (e.g. local `astro dev`, whose Miniflare doesn't inject it). Same fail-open
  // rule as a missing binding above — an auxiliary abuse guard must never take the chat down.
  const ip = getClientIP(request);
  if (!ip) {
    console.warn('[chat] no client IP on request; skipping rate limit');
    return { ok: true };
  }
