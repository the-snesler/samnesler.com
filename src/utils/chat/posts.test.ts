...
describe('mdxToPlainText', () => {
  ...
  it('keeps prose nested inside a wrapper element, even on the same line as the tags', () => {
    const result = mdxToPlainText(['<div class="grid gap-4">', '  <Card>**Desktop** OS: Windows 11</Card>', '</div>'].join('\n'));
    expect(result).toBe('**Desktop** OS: Windows 11');
  });

  it('does not rewrite link/image syntax or strip self-closing tags inside a code fence', () => {
    const result = mdxToPlainText(['```html', '<br/>', 'see [a](b)', '```'].join('\n'));
    expect(result).toBe('```\n<br/>\nsee [a](b)\n```');
  });
